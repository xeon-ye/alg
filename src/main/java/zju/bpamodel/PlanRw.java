...
public class PlanRw {
    ...
    public static void parseAndSave(String data, String filePath, String dbFile) {
        ...
        String TABLE_DATA_NAME = "Plan";
        String sql = "delete from " + TABLE_DATA_NAME + " where data='" + data + "'";
        sqliteDb.executeSql(sql);
        for (int i = 0; i < keys.length; i++) {
            double[] plans = (double[]) hash.get(keys[i]);
            StringBuilder insertSql = new StringBuilder("insert into " + TABLE_DATA_NAME + " values('" + data + "','" + keys[i] + "',");
