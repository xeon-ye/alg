 ...
 */
public class AvailableCapOptNew extends BranchBasedModel {

    private static Logger log = LogManager.getLogger(AvailableCapOptNew.class);
    ...
    public AvailableCapOptNew(DistriSys sys) {
        super(sys);
    }
...
//        }
        try {
            ...
            // 约束方程系数
            double[][] coeff = new double[1 + loops.size() + nodes.size() + 2 * edges.size()][objValue.length];
            // 所有支路的通断状态
...
//                    }
    ...
    public void doOptPathLimit() {
        ...
        IloNumVarType[] xt  = new IloNumVarType[objValue.length];
        // 约束方程系数
        double[][] coeff = new double[1 + loops.size() + nodes.size() + 2 * edges.size() + impPaths.size()][objValue.length];
        // 所有支路的通断状态
        int[] edgesStatues = new int[edges.size()];
        //记录数组中存储元素的个数
        int coeffNum = 0;
...
//                objValue[columnLower.length - 1] = 1;
                for (MapObject e : g.edgesOf(loadNodes.get(loadIndex))) {
                    for (j = 0; j < edges.size(); j++) {
                        if (edges.get(j).equals(e)) {
                            if (g.getEdgeTarget(e).equals(loadNodes.get(loadIndex))) {
                                objValue[edges.size() + j] = 1;
                            } else {
                                objValue[edges.size() + j] = -1;
                            }
                            break;
                ...
                for(i = 0; i < loadNodes.size(); i++) {
                    if (i != loadIndex) {
                        ...
                        if (i == loadIndex) {
                            cplex.addGe(cplex.scalProd(x, coeff[coeffNum]), loadArray[i]);
                        } else {
                            cplex.addEq(cplex.scalProd(x, coeff[coeffNum]), loadArray[i]);
                        }
                        coeffNum++;
                ...
                if (cplex.getStatus().equals(IloCplex.Status.Optimal) && maxLoad > cplex.getObjValue() - loadArray[loadIndex]) {
                    maxLoad = cplex.getObjValue() - loadArray[loadIndex];
