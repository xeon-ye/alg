...
/**
 * Created by xuchengsi on 2018/3/13.
 */
public class AvailableCapOptVF extends BranchBasedModel {
    ...
    public AvailableCapOptVF(DistriSys sys) {
        super(sys);
    }
    ...
    public void doOpt() {
            ...
            for (j = 0; j < errorSupply.length; j++) {
                if (supplies[i].equals(errorSupply[j])) {
                    ...
                    for (MapObject e : g.edgesOf(sys.getCns().get(supplies[i]))) {
                        ...
                        feederCapacity[edges.indexOf(e)] = 0;
                    }
                    break;
                }
            }
        ...
        try {
            // 开始构造线性规划模型
            double objValue[] = new double[2 * edges.size() + edges.size()];
            ...
            // 约束方程系数
            double[][] coeff = new double[1 + loadNodes.size() + 2 * edges.size() + nodes.size() + 2 * edges.size()][objValue.length];
            // 所有支路的通断状态
            ...
            // 虚拟流量变量上下限
            for (i = 0; i < edges.size(); i++) {
                columnLower[2 * edges.size() + i] = -loadNodes.size();
                columnUpper[2 * edges.size() + i] = loadNodes.size();
            ...
            for (i = 0; i < loadNodes.size(); i++) {
                for (MapObject e : g.edgesOf(loadNodes.get(i))) {
                    ...
                    if (g.getEdgeTarget(e).equals(loadNodes.get(i))) {
                        coeff[coeffNum][2 * edges.size() + j] = 1;
                    } else {
                        coeff[coeffNum][2 * edges.size() + j] = -1;
                    }
                }
            ...
            for(i = 0; i < edges.size(); i++) {
                ...
                coeff[coeffNum][i] = -loadNodes.size();
    ...
    public void doOptPathLimit() {
        ...
        // 开始构造线性规划模型
        double objValue[] = new double[2 * edges.size() + 2 * impPaths.size() + edges.size()];
        // 状态变量下限, column里元素的个数等于矩阵C里系数的个数
        double columnLower[] = new double[objValue.length];
        // 状态变量上限
        double columnUpper[] = new double[objValue.length];
        ...
        IloNumVarType[] xt  = new IloNumVarType[objValue.length];
        // 约束方程系数
        double[][] coeff = new double[1 + loadNodes.size() + 2 * edges.size() + nodes.size() + 2 * edges.size() + impPaths.size()][objValue.length];
        // 所有支路的通断状态
        int[] edgesStatues = new int[edges.size()];
        //记录数组中存储元素的个数
        int coeffNum = 0;
...
//            columnUpper[2 * edges.size() + impPaths.size() + i] = 1 - Double.MIN_VALUE;
        ...
        // 虚拟流量变量上下限
        for (i = 0; i < edges.size(); i++) {
            columnLower[2 * edges.size() + 2 * impPaths.size() + i] = -loadNodes.size();
            columnUpper[2 * edges.size() + 2 * impPaths.size() + i] = loadNodes.size();
        ...
        try {
            ...
            for (i = 0; i < loadNodes.size(); i++) {
                for (MapObject e : g.edgesOf(loadNodes.get(i))) {
                    j = edges.indexOf(e);
                    if (g.getEdgeTarget(e).equals(loadNodes.get(i))) {
                        coeff[coeffNum][2 * edges.size() + 2 * impPaths.size() + j] = 1;
                    } else {
                        coeff[coeffNum][2 * edges.size() + 2 * impPaths.size() + j] = -1;
                    }
                }
