...
public class IesPlan {
    ...
    public void doOpt() {
        ...
        try {
            ...
            // 约束方程系数
            double[][] coeff = new double[1 + nodes.size() + 2 * edges.size()][columnLower.length];
            // 所有支路的通断状态
            ...
            // 支路流量变量上下限
            for (int i = 0; i < edges.size(); i++) {
                columnLower[supplies.length + edges.size() + i] = - Pem;
                columnUpper[supplies.length + edges.size() + i] = Pem;
                xt[supplies.length + edges.size() + i] = IloNumVarType.Float;
                columnLower[supplies.length + 2 * edges.size() + i] = - Pem;
                columnUpper[supplies.length + 2 * edges.size() + i] = Pem;
                xt[supplies.length + 2 * edges.size() + i] = IloNumVarType.Float;
                columnLower[supplies.length + 3 * edges.size() + i] = - Phm;
                columnUpper[supplies.length + 3 * edges.size() + i] = Phm;
            ...
            for (int i = 0; i < supplies.length; i++) {
                obj = cplex.sum(obj, cplex.prod(coef1, x[i]));
            }
            ...
            for (int i = 0; i < supplyNodes.size(); i++) {
                ...
                coeff[coeffNum][i] = Se;
                coeff[coeffNum + 1][i] = Se;
                coeff[coeffNum][i + 2] = Sh;
            ...
            for (int i = 0; i < edges.size(); i++) {
                if (result[supplies.length + i] == 1) {
                    System.out.printf("%s\t", edges.get(i).getId());
//                    System.out.println(edges.get(i).getProperties(KEY_CONNECTED_NODE));
