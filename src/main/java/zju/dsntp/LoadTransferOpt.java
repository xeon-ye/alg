...
//            System.out.printf("%3d ", column[i]);
    ...
    public void doOptLoadMax() {
        ...
        columnUpper[columnLower.length-1] = Double.MAX_VALUE;
        //设置变量的系数值
        ...
        for(i = 0; i < loadPathes.size(); i++) {
            ...
            rowLower[rowLowerLen++] = -Double.MAX_VALUE;
            element[elementLen++] = 1;
            element[elementLen++] = -Double.MAX_VALUE;
            column[columnLen++] = pathes.size()+i;
            column[columnLen++] = cnpathesIndex.get(cnStart[loadIndex]+i);
            //Z >= -WM
            starts[startsLen++] = elementLen;
            rowUpper[rowUpperLen++] = Double.MAX_VALUE;
            rowLower[rowLowerLen++] = 0;
            element[elementLen++] = 1;
            element[elementLen++] = Double.MAX_VALUE;
            column[columnLen++] = pathes.size()+i;
