...
//            jsdkyOpt(user);
    ...
    public void jsdkyOpt(User user) {
        List<AbsorptionChiller> absorptionChillers = user.getAbsorptionChillers();
        List<AirCon> airCons = user.getAirCons();
        List<Converter> converters = user.getConverters();
        List<GasBoiler> gasBoilers = user.getGasBoilers();
        ...
        List<Storage> storages = user.getStorages();
        Photovoltaic photovoltaic = user.getPhotovoltaic();
        WindPower windPower = user.getWindPower();
        double[] acLoad = user.acLoad;
        double[] dcLoad = user.dcLoad;
        double[] heatLoad1 = user.heatLoad1;
        double[] heatLoad2 = user.heatLoad2;
        double[] coolingLoad1 = user.coolingLoad1;
        double[] coolingLoad2 = user.coolingLoad2;
        double[] gatePowers = user.gatePowers;
        List<HeatStorage> heatstorages = user.heatStorages;
        try {
            // 变量：制冷机耗电功率，蓄冰槽耗电功率，蓄冰槽制冷功率(Q)，燃气轮机启停状态，表示燃气轮机状态变化的变量，
            // 燃气轮机产电功率，储能充电功率(外部)，储能放电功率(外部)，变流器AC-DC交流侧功率，变流器DC-AC交流侧功率，
            // 电网输入电功率，向电网购电功率，中央空调制冷耗电功率，中央空调制热耗电功率，燃气锅炉启停状态，表示燃气锅炉状态变化的变量，
            // 燃气锅炉产热功率，吸收式制冷机耗热功率，园区输入热功率，向园区购热功率，储热罐储热功率(外部)，储热罐放热功率(外部)，储热罐储热量
            int periodVarNum = 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                    2 * converters.size() + 2 + 2 * airCons.size() + 3 * gasBoilers.size() + absorptionChillers.size() +
                    2 + 3 * heatstorages.size();
            int varNum = periodNum * periodVarNum;
            // 状态变量下限, column里元素的个数等于矩阵C里系数的个数
            double columnLower[] = new double[varNum];
            // 状态变量上限
            ...
            IloNumVarType[] xt = new IloNumVarType[varNum];
            // 约束方程系数
            double[][] coeff = new double[(1 + 1 + 1) * periodNum + (2 + 2) * gasTurbines.size() * periodNum +
                    2 * gasTurbines.size() * (periodNum - 1) + (2 + 2) * gasBoilers.size() * periodNum +
                    2 * gasBoilers.size() * (periodNum - 1) + 4 * storages.size() * (periodNum - 1) +
                    2 * storages.size() * (periodNum - 1) + storages.size() +
                    iceStorageAcs.size() * periodNum +
                    2 * iceStorageAcs.size() * (periodNum - 1) +
                    iceStorageAcs.size() + heatstorages.size() * periodNum + heatstorages.size() +
                    5 * periodNum][varNum];
            ...
            for (int j = 0; j < periodNum; j++) {
                ...
                for (int i = 0; i < airCons.size(); i++) {
                    ...
                    columnUpper[j * periodVarNum + handledVarNum + i] = airCons.get(i).getMaxP();
                    xt[j * periodVarNum + handledVarNum + i] = IloNumVarType.Float;
                    columnLower[j * periodVarNum + handledVarNum + airCons.size() + i] = airCons.get(i).getMinPh();
                    columnUpper[j * periodVarNum + handledVarNum + airCons.size() + i] = airCons.get(i).getMaxPh();
                    xt[j * periodVarNum + handledVarNum + airCons.size() + i] = IloNumVarType.Float;
                }

                handledVarNum += 2 * airCons.size();
                for (int i = 0; i < gasBoilers.size(); i++) {
                    columnLower[j * periodVarNum + handledVarNum + i] = 0;
                    columnUpper[j * periodVarNum + handledVarNum + i] = 1;
                    xt[j * periodVarNum + handledVarNum + i] = IloNumVarType.Bool;
                ...
                handledVarNum += absorptionChillers.size();
                columnLower[j * periodVarNum + handledVarNum] = 0;
                columnUpper[j * periodVarNum + handledVarNum] = 1;
                xt[j * periodVarNum + handledVarNum] = IloNumVarType.Float;
                ...
                for (int i = 0; i < heatstorages.size(); i++) {
                    columnLower[j * periodVarNum + handledVarNum + i] = 0;
                    columnUpper[j * periodVarNum + handledVarNum + i] = heatstorages.get(i).maxPin;
                    xt[j * periodVarNum + handledVarNum + i] = IloNumVarType.Float;
                    columnLower[j * periodVarNum + handledVarNum + heatstorages.size() + i] = 0;
                    columnUpper[j * periodVarNum + handledVarNum + heatstorages.size() + i] = heatstorages.get(i).maxPout;
                    xt[j * periodVarNum + handledVarNum + heatstorages.size() + i] = IloNumVarType.Float;
                    columnLower[j * periodVarNum + handledVarNum + 2 * heatstorages.size() + i] = heatstorages.get(i).minS;
                    columnUpper[j * periodVarNum + handledVarNum + 2 * heatstorages.size() + i] = heatstorages.get(i).maxS;
                    xt[j * periodVarNum + handledVarNum + 2 * heatstorages.size() + i] = IloNumVarType.Float;
                }
            }
            ...
            for (int j = 0; j < periodNum; j++) {
                ...
                handledVarNum += 1;
                for (int i = 0; i < airCons.size(); i++) {
                    objValue[j * periodVarNum + handledVarNum + i] = airCons.get(i).getCoper() * t;   // 制冷运维成本
                    objValue[j * periodVarNum + handledVarNum + airCons.size() + i] = airCons.get(i).getCoper() * t;   // 制热运维成本
                }
            ...
            for (int j = 0; j < periodNum; j++) {
                ...
                for (int i = 0; i < airCons.size(); i++) {
                    coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                            2 * converters.size() + 2 + i] = - airCons.get(i).getConsumCoef();   // 空调制冷耗电功率
                    coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                            2 * converters.size() + 2 + airCons.size() + i] = - airCons.get(i).getConsumCoef();   // 空调制热耗电功率
                }
                for (int i = 0; i < heatstorages.size(); i++) {
                    coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                            2 * converters.size() + 2 + 2 * airCons.size() + 3 * gasBoilers.size() + absorptionChillers.size() +
                ...
                cplex.addEq(cplex.scalProd(x, coeff[coeffNum]), dcLoad[j]);
                coeffNum += 1;
                ...
                for (int i = 0; i < gasBoilers.size(); i++) {
                    coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                            2 * converters.size() + 2 + 2 * airCons.size() + 2 * gasBoilers.size() + i] = 1; // 燃气锅炉产电功率
                    coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                            2 * converters.size() + 2 + 2 * airCons.size() + i] = - gasBoilers.get(i).getMaxH();
                    cplex.addLe(cplex.scalProd(x, coeff[coeffNum]), 0);
                    coeffNum += 1;
                    coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                            2 * converters.size() + 2 + 2 * airCons.size() + 2 * gasBoilers.size() + i] = 1; // 燃气锅炉产电功率
                    coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                            2 * converters.size() + 2 + 2 * airCons.size() + i] = - gasBoilers.get(i).getMinH();
                    cplex.addGe(cplex.scalProd(x, coeff[coeffNum]), 0);
                    coeffNum += 1;
                }
                ...
                if (j == 0) {
                    for (int i = 0; i < gasBoilers.size(); i++) {
                        coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                                2 * converters.size() + 2 + 2 * airCons.size() + gasBoilers.size() + i] = 1; // 燃气锅炉状态变化
                        coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() +
                                2 * storages.size() + 2 * converters.size() + 2 + 2 * airCons.size() + i] = - 1; // 燃气锅炉状态
                        cplex.addGe(cplex.scalProd(x, coeff[coeffNum]), - gasBoilers.get(i).getInitState());
                        coeffNum += 1;
                        coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                                2 * converters.size() + 2 + 2 * airCons.size() + gasBoilers.size() + i] = 1; // 燃气锅炉状态变化
                    ...
                    for (int i = 0; i < gasBoilers.size(); i++) {
                        ...
                        coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() +
                                2 * storages.size() + 2 * converters.size() + 2 + 2 * airCons.size() + i] = - 1; // 燃气锅炉状态
                        coeff[coeffNum][(j - 1) * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() +
                                2 * storages.size() + 2 * converters.size() + 2 + 2 * airCons.size() + i] = 1; // 燃气锅炉上一时刻状态
                        cplex.addGe(cplex.scalProd(x, coeff[coeffNum]), 0);
                        coeffNum += 1;
                        coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                                2 * converters.size() + 2 + 2 * airCons.size() + gasBoilers.size() + i] = 1; // 燃气锅炉状态变化
                ...
                if (j > 0) {
                    for (int i = 0; i < gasBoilers.size(); i++) {
                        coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                                2 * converters.size() + 2 + 2 * airCons.size() + 2 * gasBoilers.size() + i] = 1; // 燃气锅炉产热功率
                        coeff[coeffNum][(j - 1) * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                                2 * converters.size() + 2 + 2 * airCons.size() + 2 * gasBoilers.size() + i] = - 1;   // 上一时刻燃气锅炉产热功率
                        cplex.addLe(cplex.scalProd(x, coeff[coeffNum]), gasBoilers.get(i).getRampRate());
                        coeffNum += 1;
                        coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                                2 * converters.size() + 2 + 2 * airCons.size() + 2 * gasBoilers.size() + i] = 1; // 燃气锅炉产热功率
            ...
            for (int j = 0; j < periodNum; j++) {
                for (int i = 0; i < heatstorages.size(); i++) {
                    coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                            2 * converters.size() + 2 + 2 * airCons.size() + 3 * gasBoilers.size() + absorptionChillers.size() +
                            2 + i] = - t * heatstorages.get(i).Effin; // 储热罐蓄热量
                    coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                            2 * converters.size() + 2 + 2 * airCons.size() + 3 * gasBoilers.size() + absorptionChillers.size() +
                            2 + heatstorages.size() + i] = t / heatstorages.get(i).getEffout(); // 储热罐放热量
                    coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                            2 * converters.size() + 2 + 2 * airCons.size() + 3 * gasBoilers.size() + absorptionChillers.size() +
                            2 + 2 * heatstorages.size() + i] = 1; // 储热罐储热量
                    if (j == 0) {
                        cplex.addEq(cplex.scalProd(x, coeff[coeffNum]), heatstorages.get(i).initS);
                    } else {
                        coeff[coeffNum][(j - 1) * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                                2 * converters.size() + 2 + 2 * airCons.size() + 3 * gasBoilers.size() + absorptionChillers.size() +
                                2 + 2 * heatstorages.size() + i] = - 1; // 上一时刻储热罐储热量
            ...
            // 储热罐日累积热量约束
            for (int i = 0; i < heatstorages.size(); i++) {
                coeff[coeffNum][(periodNum - 1) * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                        2 * converters.size() + 2 + 2 * airCons.size() + 3 * gasBoilers.size() + absorptionChillers.size() +
                        2 + 2 * heatstorages.size() + i] = 1; // 储热罐储热量
                cplex.addEq(cplex.scalProd(x, coeff[coeffNum]), heatstorages.get(i).initS);
                coeffNum += 1;
            ...
            for (int j = 0; j < periodNum; j++) {
                ...
                for (int i = 0; i < absorptionChillers.size(); i++) {
                    coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                            2 * converters.size() + 2 + 2 * airCons.size() + 3 * gasBoilers.size() + i] = - 1;   // 吸收式制冷机耗热功率
                }
                coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                        2 * converters.size() + 2 + airCons.size()] = airCons.get(0).EERh;   // 地源热泵制热功率
                coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                        2 * converters.size() + 2 + airCons.size() + 1] = airCons.get(1).EERh;   // 空调制热功率
                for (int i = 0; i < heatstorages.size(); i++) {
                    coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                            2 * converters.size() + 2 + 2 * airCons.size() + 3 * gasBoilers.size() + absorptionChillers.size() +
                            2 + i] = - 1;   // 储热罐储热功率
                    coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                            2 * converters.size() + 2 + 2 * airCons.size() + 3 * gasBoilers.size() + absorptionChillers.size() +
                            2 + heatstorages.size() + i] = 1;   // 储热罐放热功率
                }
                cplex.addGe(cplex.scalProd(x, coeff[coeffNum]), heatLoad1[j] - photovoltaic.heatPower[j]);
                coeffNum += 1;
            }
            ...
            for (int j = 0; j < periodNum; j++) {
                coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                            2 * converters.size() + 2 + airCons.size() + 2] = airCons.get(2).EERh;   // 空气源热泵制热功率
                cplex.addGe(cplex.scalProd(x, coeff[coeffNum]), heatLoad2[j]);
                coeffNum += 1;
            }
            ...
            for (int j = 0; j < periodNum; j++) {
                ...
                coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                        2 * converters.size() + 2] = airCons.get(0).EERc;   // 地源热泵制冷功率
                coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                        2 * converters.size() + 2 + 1] = airCons.get(1).EERc;   // 空调制冷功率
                for (int i = 0; i < absorptionChillers.size(); i++) {
                    coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                            2 * converters.size() + 2 + 2 * airCons.size() + 3 * gasBoilers.size() + i] = absorptionChillers.get(i).getIc();   // 吸收式制冷机供冷功率
                }
            ...
            for (int j = 0; j < periodNum; j++) {
                coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                        2 * converters.size() + 2 + 2] = airCons.get(2).EERc;   // 空气源热泵制冷功率
            ...
            if (cplex.solve()) {
                ...
                if (cplex.getStatus() == IloCplex.Status.Optimal) {
                    ...
                    for (int j = 0; j < periodNum; j++) {
                        minCost += photovoltaic.getCoper() * photovoltaic.getPower()[j];
                        minCost += windPower.getCoper() * windPower.getPower()[j];
                    }
                    createDkyUserResult(user.getUserId(), cplex.getStatus().toString(), minCost, cplex.getValues(x),
                            periodVarNum, iceStorageAcs, gasTurbines, storages, converters, airCons, gasBoilers,
                            absorptionChillers, heatstorages);
                } else {
                    UserResult userResult = new UserResult(user.getUserId(), cplex.getStatus().toString());
                    microgridResult.put(user.getUserId(), userResult);
                }
    ...
    public void gzTestSelfOpt(User user) {
        List<AbsorptionChiller> absorptionChillers = user.getAbsorptionChillers();
        List<AirCon> airCons = user.getAirCons();
        List<Converter> converters = user.getConverters();
        List<GasBoiler> gasBoilers = user.getGasBoilers();
        ...
        try {
            ...
            int periodVarNum = 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                    2 * converters.size() + 2 + airCons.size() + 3 * gasBoilers.size() + absorptionChillers.size() + 2 + storages.size();
            int varNum = periodNum * periodVarNum;
            // 状态变量下限, column里元素的个数等于矩阵C里系数的个数
            double columnLower[] = new double[varNum];
            // 状态变量上限
            ...
            for (int j = 0; j < periodNum; j++) {
                ...
                coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() + 2 * converters.size() + 2] = airCons.get(0).getEffac() * airCons.get(0).getEERc();   // 空调1制冷功率
                for (int i = 0; i < absorptionChillers.size(); i++) {
                    coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() + 2 * converters.size() + 2 + airCons.size() + 3 * gasBoilers.size() + i] = absorptionChillers.get(i).getIc();   // 吸收式制冷机供冷功率
                }
                cplex.addEq(cplex.scalProd(x, coeff[coeffNum]), coolingLoad1[j]);
            ...
            for (int j = 0; j < periodNum; j++) {
                ...
                coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() + 2 * converters.size() + 2 + 1] = airCons.get(1).getEffac() * airCons.get(1).getEERc();   // 空调2制冷功率
                coeff[coeffNum][j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() + 2 * converters.size() + 2 + 2] = airCons.get(2).getEffac() * airCons.get(2).getEERc();   // 空调3制冷功率
                cplex.addEq(cplex.scalProd(x, coeff[coeffNum]), coolingLoad2[j]);
                coeffNum += 1;
            }
    ...
    public void createDkyUserResult(String userId, String status, double minCost, double[] result, int periodVarNum,
                                 List<IceStorageAc> iceStorageAcs, List<GasTurbine> gasTurbines, List<Storage> storages,
                                 List<Converter> converters, List<AirCon> airCons, List<GasBoiler> gasBoilers,
                                 List<AbsorptionChiller> absorptionChillers, List<HeatStorage> heatStorages) {
        List<double[]> frigesP = new ArrayList<>(iceStorageAcs.size());
        List<double[]> iceTanksP = new ArrayList<>(iceStorageAcs.size());
        List<double[]> iceTanksQ = new ArrayList<>(iceStorageAcs.size());
        for (int i = 0; i < iceStorageAcs.size(); i++) {
        ...
        double[] purP = new double[periodNum];
        List<double[]> airConsP = new ArrayList<>(airCons.size());
        List<double[]> airConsPh = new ArrayList<>(airCons.size());
        for (int i = 0; i < airCons.size(); i++) {
            airConsP.add(new double[periodNum]);
            airConsPh.add(new double[periodNum]);
        }
        List<double[]> gasBoilersState = new ArrayList<>(gasBoilers.size());
        List<double[]> gasBoilersH = new ArrayList<>(gasBoilers.size());
        for (int i = 0; i < gasBoilers.size(); i++) {
        ...
        double[] Hin = new double[periodNum];
        double[] purH = new double[periodNum];
        List<double[]> heatStoragesP = new ArrayList<>(heatStorages.size());
        List<double[]> heatStoragesS = new ArrayList<>(heatStorages.size());
        for (int i = 0; i < heatStorages.size(); i++) {
            heatStoragesP.add(new double[periodNum]);
            heatStoragesS.add(new double[periodNum]);
        }
        for (int j = 0; j < periodNum; j++) {
            for (int i = 0; i < iceStorageAcs.size(); i++) {
                frigesP.get(i)[j] = result[j * periodVarNum + i];
            ...
            purP[j] = result[j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() + 2 * converters.size() + 1];
            for (int i = 0; i < airCons.size(); i++) {
                airConsP.get(i)[j] = result[j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() +
                        2 * storages.size() + 2 * converters.size() + 2 + i];
                airConsPh.get(i)[j] = result[j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() +
                        2 * storages.size() + 2 * converters.size() + 2 + airCons.size() + i];
            }
            for (int i = 0; i < gasBoilers.size(); i++) {
                gasBoilersState.get(i)[j] = result[j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() +
                        2 * storages.size() + 2 * converters.size() + 2 + 2 * airCons.size() + i];
                gasBoilersH.get(i)[j] = result[j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() +
                        2 * storages.size() + 2 * converters.size() + 2 + 2 * airCons.size() + 2 * gasBoilers.size() + i];
            }
            for (int i = 0; i < absorptionChillers.size(); i++) {
                absorptionChillersH.get(i)[j] = result[j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() +
                        2 * storages.size() + 2 * converters.size() + 2 + 2 * airCons.size() + 3 * gasBoilers.size() + i];
            }
            Hin[j] = result[j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                    2 * converters.size() + 2 + 2 * airCons.size() + 3 * gasBoilers.size() + absorptionChillers.size()];
            purH[j] = result[j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                    2 * converters.size() + 2 + 2 * airCons.size() + 3 * gasBoilers.size() + absorptionChillers.size() + 1];
            for (int i = 0; i < heatStorages.size(); i++) {
                heatStoragesP.get(i)[j] = result[j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() +
                        2 * storages.size() + 2 * converters.size() + 2 + 2 * airCons.size() + 3 * gasBoilers.size() + absorptionChillers.size() + 2 + i] -
                        result[j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                                2 * converters.size() + 2 + 2 * airCons.size() + 3 * gasBoilers.size() + absorptionChillers.size() +
                                2 + heatStorages.size() + i];
                heatStoragesS.get(i)[j] = result[j * periodVarNum + 3 * iceStorageAcs.size() + 3 * gasTurbines.size() + 2 * storages.size() +
                                2 * converters.size() + 2 + 2 * airCons.size() + 3 * gasBoilers.size() + absorptionChillers.size() +
                                2 + 2 * heatStorages.size() + i];
            }
        }
        UserResult userResult = new UserResult(userId, status, minCost,
                frigesP, iceTanksP, iceTanksQ, gasTurbinesState, gasTurbinesP, storagesP, convertersP, Pin,
                purP, airConsP, gasBoilersState, gasBoilersH, absorptionChillersH, Hin, purH);
        userResult.setAirConsPh(airConsPh);
        userResult.setHeatStoragesP(heatStoragesP);
        userResult.setHeatStoragesS(heatStoragesS);
        microgridResult.put(userId, userResult);
    }
