 ...
 * @date 2019/6/20
 */
public class DispatchOptModel {

    Microgrid microgrid;
    double dispatchTime;   // 调度时长（h）
    int periodNum; // 时段数
    double t;   // 单位时段长度
    ...
    public DispatchOptModel(Microgrid microgrid, double dispatchTime, int periodNum, double[] elecPrices, double[] gasPrices, double[] steamPrices) {
        this.microgrid = microgrid;
        this.dispatchTime = dispatchTime;
        this.periodNum = periodNum;
        t = dispatchTime / periodNum;
        this.elecPrices = elecPrices;
        this.gasPrices = gasPrices;
        this.steamPrices = steamPrices;
    }

    public void mgDispatchOpt() {
        microgridResult = new HashMap<>(microgrid.getUsers().size());
        for (User user : microgrid.getUsers().values()) {
            userDispatchOpt(user);
        }
    }

    public void userDispatchOpt(User user) {
        ...
        try {
            ...
            for (int j = 0; j < periodNum; j++) {
                ...
                periodDcLoad -= windPower.getPower()[j];
