...
public abstract class Ipopt {
    ...
    /// Callback arguments
    protected double callback_grad_f[];
    protected double callback_jac_g[];
    protected double callback_hess[];

    protected double[] x;
    /**
     * Final value of objective function
     */
    protected double obj_val[] = {0.0};
     ...
     */
    protected double mult_x_L[];

    /**
     * Final multipliers for upper variable bounds
     */
    protected double mult_x_U[];

    /**
     * Final multipliers for constraints
     */
    protected double mult_g[];

    /**
     * Status returned by the solver
     */
    protected int status = INVALID_PROBLEM_DEFINITION;
    ...
    protected boolean setIntegerOption(String keyword, int val) {
        return ipopt != 0 && AddIpoptIntOption(ipopt, keyword, val);
    }
    ...
    protected boolean setNumericOption(String keyword, double val) {
        return ipopt != 0 && AddIpoptNumOption(ipopt, keyword, val);
    }
    ...
    protected boolean setStringOption(String keyword, String val) {
        return ipopt != 0 && AddIpoptStrOption(ipopt, keyword, val.toLowerCase());
    }
     ...
     * @return true means success, false means fail!
     */
    public boolean get_list_of_nonlinear_variables(int num_nonlin_vars, int[] pos_nonlin_vars) {
        return false;
    }
